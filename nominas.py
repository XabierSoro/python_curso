...
class Analista(Empleado): # Hacemos una nueva clase
    def __init__(self, nombre, apellido, puesto, pluspuesto, salariomin, nomina, rango):# En el constructor añado el rango, esto servirá para calcular el sueldo
        super().__init__(nombre, apellido, puesto, pluspuesto, salariomin, nomina)#Esto hereda de la clase empleado
        self.rango=rango
...
class Programador(Empleado):# Hacemos una nueva clase
...
def mostrar():    
    ...
    while k< len(empleados):
        ...
        print(f"El empleado {empleados[k].nombre} es {empleados[k].puesto} y cobra {empleados[k].nomina}€")
        k+=1
